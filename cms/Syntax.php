...
class Syntax {
	...
	function syntax_kategorie($desciption, $value) {
		...
		if ($CatPage->exists_CatPage($cat, false)) {
			return $CatPage->create_LinkTag($CatPage->get_Href($cat, false), $link_text, "category", $language->getLanguageValue1("tooltip_link_category_1", $CatPage->get_HrefText($cat, false)));
		} else {
	...
	function syntax_seite($desciption, $value) {
		...
		return $CatPage->create_LinkTag($CatPage->get_Href($cat, $page), $link_text, "page", $language->getLanguageValue2("tooltip_link_page_2", $CatPage->get_HrefText($cat, $page), $CatPage->get_HrefText($cat, false)));
	}
	...
	function syntax_datei($desciption, $value) {
		...
		if ($CMS_CONF->get("targetblank_download") == "true")
			$target = "_blank";
		return $CatPage->create_LinkTag($CatPage->get_HrefFile($cat, $datei), $link_text, "file", $language->getLanguageValue2("tooltip_link_file_2", $CatPage->get_FileText($cat, $datei), $CatPage->get_HrefText($cat, false)), $target);
	}
	...
	function replaceAnchorAbsatz() {
		...
		foreach ($GLOBALS ['syntax_anchor_absatz'] as $pos => $ueber_array) {
			...
			if ($ueber_array ['ueber'] == $language->getLanguageValue0("anchor_top_0") or $ueber_array ['ueber'] == "_absatztop-") {
