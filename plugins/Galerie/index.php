...
class Galerie extends Plugin {
	...
	function getContent($value) {
		...
		if (($embedded == "_self") or (getRequestParam('gal', false))) {
			...
			return "<a class=\"gallery\" href=\"" . $linkprefix . "gal=" . $gal_request . "\" " . getTitleAttribute($lang_gallery_cms->getLanguageValue2("tooltip_link_gallery_2", $specialchars->rebuildSpecialChars($values [0], false, true), $j)) . "target=\"" . $this->settings->get("target") . "\">" . $gal_name . "</a>";
		}
	}
	...
	function getGalleryMenu($picarray, $linkprefix, $gal_request, $index, $first, $previous, $next, $last) {
		...
		$gallerymenu .= "<li class=\"gallerymenu\"><a href=\"" . $linkprefix . "gal=" . $gal_request . "&amp;index=" . $first . "\" class=\"$linkclass\">" . $lang_gallery_cms->getLanguageValue0("message_firstimage_0") . "</a></li>";
		// Link "Voriges Bild"
		$gallerymenu .= "<li class=\"gallerymenu\"><a href=\"" . $linkprefix . "gal=" . $gal_request . "&amp;index=" . $previous . "\" class=\"gallerymenu\">" . $lang_gallery_cms->getLanguageValue0("message_previousimage_0") . "</a></li>";
		// Link "Nächstes Bild"
		$gallerymenu .= "<li class=\"gallerymenu\"><a href=\"" . $linkprefix . "gal=" . $gal_request . "&amp;index=" . $next . "\" class=\"gallerymenu\">" . $lang_gallery_cms->getLanguageValue0("message_nextimage_0") . "</a></li>";
		// Link "Letztes Bild"
		if ($index == $last)
			$linkclass = "gallerymenuactive";
		else
			$linkclass = "gallerymenu";
		$gallerymenu .= "<li class=\"gallerymenu\"><a href=\"" . $linkprefix . "gal=" . $gal_request . "&amp;index=" . $last . "\" class=\"$linkclass\">" . $lang_gallery_cms->getLanguageValue0("message_lastimage_0") . "</a></li>";
		// Rückgabe des Menüs
	...
	function getNumberMenu($picarray, $linkprefix, $index, $gal_request, $first, $last) {
		...
		for($i = $first; $i <= $last; $i++) {
			...
			$numbermenu .= "<li class=\"gallerynumbermenu\">" . "<a href=\"" . $linkprefix . "gal=" . $gal_request . "&amp;index=" . $i . "\" class=\"" . $cssclass . "\">" . $i . "</a>" . "</li>";
		}
		// Rückgabe des Menüs
		$numbermenu .= "</ul>";
		return $numbermenu;
	...
	function getThumbnails($picarray, $alldescriptions, $GALERIE_DIR, $GALERIE_DIR_SRC) {
		...
		for($i = 0; $i < count($picarray); $i++) {
			...
			if (file_exists($GALERIE_DIR . PREVIEW_DIR_NAME . "/" . $specialchars->replaceSpecialChars($picarray [$i], false))) {
				...
				$thumbs .= '<div style="text-align:center;"><a style="color:red;" href="' . $GALERIE_DIR_SRC . PREVIEW_DIR_NAME . "/" . $specialchars->replaceSpecialChars($picarray [$i], true) . '" target="_blank" title="' . $lang_gallery_cms->getLanguageValue1("tooltip_gallery_fullscreen_1", $specialchars->rebuildSpecialChars($picarray [$i], true, true)) . '"><b>' . $lang_gallery_cms->getLanguageValue0('message_gallery_no_preview') . '</b></a></div>';
			}
			$thumbs .= $description . "</td>";
		}
	...
	function getCurrentPic($picarray, $index, $GALERIE_DIR_SRC) {
		...
		$currentpic = "<a href=\"" . $GALERIE_DIR_SRC . $specialchars->replaceSpecialChars($picarray [$index - 1], true) . "\" target=\"_blank\" title=\"" . $lang_gallery_cms->getLanguageValue1("tooltip_gallery_fullscreen_1", $specialchars->rebuildSpecialChars($picarray [$index - 1], true, true)) . "\">";
		$currentpic .= "<img src=\"" . $GALERIE_DIR_SRC . $specialchars->replaceSpecialChars($picarray [$index - 1], true) . "\" alt=\"" . $lang_gallery_cms->getLanguageValue1("alttext_galleryimage_1", $specialchars->rebuildSpecialChars($picarray [$index - 1], true, true)) . "\" />";
	...
	function getInfo() {
		...
		$info = array(
			// Plugin-Name
			"<b>" . $lang_gallery_admin->get("config_gallery_plugin_name") . "</b> \$Revision: 149 $",
			// CMS-Version
			"1.12",
			// Kurzbeschreibung
