...
#        require_once($BASE_DIR_CMS."Mail.php");
        ...
        foreach(array("name","mail","website","message") as $name) {
            ${"config_".$name}[0] = $lang_contact->getLanguageValue0("contactform_".$name."_0");
        ...
        if (getRequestParam('submit', false) <> "") { 
            ...
            if (time() - $_SESSION['contactform_loadtime'] < $sendtime) {
                $errormessage = $lang_contact->getLanguageValue1("contactform_senttoofast_1", $sendtime);
            }
            if ($usespamprotection) {
                ...
                if (strtolower($calcresult) != strtolower($_SESSION['calculation_result'])) {
                    $errormessage = $lang_contact->getLanguageValue0("contactform_wrongresult_0");
                }
            }
            ...
            if ($errormessage == "") {
                ...
                if (($config_name[2] == "true") && ($name == "")) {
                    $errormessage = $lang_contact->getLanguageValue0("contactform_fieldnotset_0")." ".$config_name[0];
                }
                else if (($config_mail[2] == "true") && ($mail == "")) {
                    $errormessage = $lang_contact->getLanguageValue0("contactform_fieldnotset_0")." ".$config_mail[0];
                }
                else if (($config_website[2] == "true") && ($website == "")) {
                    $errormessage = $lang_contact->getLanguageValue0("contactform_fieldnotset_0")." ".$config_website[0];
                }
                else if (($config_message[2] == "true") && ($message == "")) {
                    $errormessage = $lang_contact->getLanguageValue0("contactform_fieldnotset_0")." ".$config_message[0];
                }
            }
            // Es ist ein Fehler aufgetreten!
            if ($errormessage <> "") {
                ...
                $mailsubject = $lang_contact->getLanguageValue1("contactform_mailsubject_1", $specialchars->getHtmlEntityDecode($WEBSITE_NAME));
                $mailsubject_confirm = $lang_contact->getLanguageValue1("contactform_mailsubject_confirm_1", $specialchars->getHtmlEntityDecode($WEBSITE_NAME));
                ...
                $form .= "<span id=\"contact_successmessage\">".$lang_contact->getLanguageValue0("contactform_confirmation_0")."</span>";
        ...
        if($usespamprotection) {
            ...
            $form .= "<tr><td colspan=\"2\">".$lang_contact->getLanguageValue0("contactform_spamprotection_text_0")."</td></tr>"
        ...
        if($mandatory)
            $form .= "<tr><td style=\"padding-right:10px;\">&nbsp;</td><td>".$lang_contact->getLanguageValue0("contactform_mandatory_fields_0")."</td></tr>";
        $form .= "<tr><td style=\"padding-right:10px;\">&nbsp;</td><td><input type=\"submit\" class=\"submit\" id=\"contact_submit\" name=\"submit\" value=\"".$lang_contact->getLanguageValue0("contactform_submit_0")."\" /></td></tr>";
        $form .= "</table>"
        ."</form>";
