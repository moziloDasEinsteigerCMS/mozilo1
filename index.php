...
function checkParameters() {
	...
	if ($CatPage->exists_CatPage($CAT_REQUEST_URL, false) and $CatPage->exists_CatPage($CAT_REQUEST_URL, $PAGE_REQUEST_URL)) {
...
function getContent() {
	...
	// Entwurf
	if ($ACTION_REQUEST == "draft" and $CatPage->get_Type(CAT_REQUEST, PAGE_REQUEST) == EXT_DRAFT and $CatPage->exists_CatPage(CAT_REQUEST, PAGE_REQUEST)) {
		$PAGE_FILE = PAGE_REQUEST . EXT_DRAFT;
		return $CatPage->get_PageContent(CAT_REQUEST, PAGE_REQUEST);
	} // normale Inhaltsseite
	elseif ($CatPage->get_Type(CAT_REQUEST, PAGE_REQUEST) == EXT_PAGE and $CatPage->exists_CatPage(CAT_REQUEST, PAGE_REQUEST)) {
		$PAGE_FILE = PAGE_REQUEST . EXT_PAGE;
		return $CatPage->get_PageContent(CAT_REQUEST, PAGE_REQUEST);
	} // Versteckte Inhaltsseite
	elseif ($CatPage->get_Type(CAT_REQUEST, PAGE_REQUEST) == EXT_HIDDEN and $CatPage->exists_CatPage(CAT_REQUEST, PAGE_REQUEST)) {
...
function getDirContentAsArray($dir, $iscatdir, $showhidden, $showdraft = false) {
	...
	foreach ($files_read as $file) {
		...
		(!$iscatdir) || (substr($file, strlen($file) - 4, strlen($file)) == EXT_PAGE) || (substr($file, strlen($file) - 4, strlen($file)) == EXT_LINK) || ($showhidden && (substr($file, strlen($file) - 4, strlen($file)) == EXT_HIDDEN)) || ($showdraft && (substr($file, strlen($file) - 4, strlen($file)) == EXT_DRAFT))) {
			$files [] = $file;
		}
	}
	sort($files);
...
function getMainMenu() {
	...
	foreach ($categoriesarray as $currentcategory) {
		$mainmenu .= '<li class="mainmenu">' . $CatPage->create_AutoLinkTag($currentcategory, false, "menu");
		if ($CatPage->is_Activ($currentcategory, false) and $CMS_CONF->get("usesubmenu") > 0) {
			$mainmenu .= getDetailMenu($currentcategory);
		} elseif (!$CatPage->is_Activ($currentcategory, false) and $CMS_CONF->get("usesubmenu") == 2) {
			$mainmenu .= getDetailMenu($currentcategory);
		}
		$mainmenu .= "</li>";
	}
...
function getDetailMenu($cat) {
	...
	if (($ACTION_REQUEST == "sitemap") && ($CMS_CONF->get("usesubmenu") == 0))
		$detailmenu .= '<li class="detailmenu">' . $CatPage->create_LinkTag($CatPage->get_Href(false, false, "action=sitemap"), $language->getLanguageValue0("message_sitemap_0"), $cssprefix . "active", false) . '</li>';
	// Suchergebnis
	elseif (($ACTION_REQUEST == "search") && ($CMS_CONF->get("usesubmenu") == 0))
		$detailmenu .= '<li class="detailmenu">' . $CatPage->create_LinkTag($CatPage->get_Href(false, false, "action=search&amp;query=" . $specialchars->replaceSpecialChars($QUERY_REQUEST, false)), $language->getLanguageValue1("message_searchresult_1", $specialchars->getHtmlEntityDecode($QUERY_REQUEST)), $cssprefix . "active", false) . '</li>';
	// Entwurfsansicht
	elseif (($ACTION_REQUEST == "draft") && ($CMS_CONF->get("usesubmenu") == 0))
		$detailmenu .= '<li class="detailmenu">' . $CatPage->create_LinkTag($CatPage->get_Href($cat, PAGE_REQUEST, "action=draft"), $CatPage->get_HrefText($cat, PAGE_REQUEST) . " (" . $language->getLanguageValue0("message_draft_0") . ")", $cssprefix . "active", false) . '</li>';
	// "ganz normales" Detailmenue einer Kategorie
		...
		foreach ($contentarray as $currentcontent) {
			$detailmenu .= '<li class="detailmenu">' . $CatPage->create_AutoLinkTag($cat, $currentcontent, $cssprefix) . "</li>";
		}
	}
	// Rueckgabe des Menues
...
function getSearchForm() {
	...
	$form = "<form accept-charset=\"" . CHARSET . "\" method=\"get\" action=\"index.php$modrewrite_dumy\" class=\"searchform\"><fieldset id=\"searchfieldset\">" . "<input type=\"hidden\" name=\"action\" value=\"search\" />" . "<input type=\"text\" name=\"query\" value=\"\" class=\"searchtextfield\" />" . "<input type=\"image\" name=\"action\" value=\"search\" src=\"" . $LAYOUT_DIR_URL . "/grafiken/searchicon.gif\" alt=\"" . $language->getLanguageValue0("message_search_0") . "\" class=\"searchbutton\"" . getTitleAttribute($language->getLanguageValue0("message_search_0")) . " />" . "</fieldset></form>";
	return $form;
}
...
function getSiteMap() {
	...
	$sitemap = "<h1>" . $language->getLanguageValue0("message_sitemap_0") . "</h1>" . "<div class=\"sitemap\">";
	// Kategorien-Verzeichnis einlesen
...
function getWebsiteTitle() {
	...
	if ($CMS_CONF->get("hidecatnamedpages") == "true" and $cat == $page and strstr($title, '{CATEGORY}') and strstr($title, '{PAGE}')) {
...
function findPlugins() {
	...
	foreach ($dircontent as $currentelement) {
		...
		if (file_exists(PLUGIN_DIR_REL . $currentelement . "/plugin.conf") and file_exists(PLUGIN_DIR_REL . $currentelement . "/index.php")) {
