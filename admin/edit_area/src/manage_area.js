	...
	EditArea.prototype.check_line_selection= function(timer_checkup){
		...
		else if(this.textareaFocused && editAreas[this.id]["displayed"]==true && this.isResizing==false)
		{
			...
			if((this.current_code_lang !="mozilo" || (this.current_code_lang=="mozilo" && infos["curr_line"].charAt(infos["curr_pos"]-2) !="^")) && (infos["line_start"] != this.last_selection["line_start"] || infos["curr_pos"] != this.last_selection["curr_pos"] || infos["full_text"].length!=this.last_selection["full_text"].length || this.reload_highlight || !timer_checkup) )
			{
				// move _cursor_pos
	...
	EditArea.prototype.findEndBracket= function(infos, bracket){
		...
		for(var i=start; i<infos["full_text"].length && i>=0; ){
			if((this.current_code_lang=="mozilo" && infos["full_text"].charAt(i-1)!="^" && infos["full_text"].charAt(i)==endBracket) || (this.current_code_lang!="mozilo" && infos["full_text"].charAt(i)==endBracket)){
				nbBracketOpen--;
				if(nbBracketOpen<=0){
			...
			}else if((this.current_code_lang=="mozilo" && infos["full_text"].charAt(i-1)!="^" && infos["full_text"].charAt(i)==bracket) || (this.current_code_lang!="mozilo" && infos["full_text"].charAt(i)==bracket))
				nbBracketOpen++;
			if(normal_order)
				i++;
			else
