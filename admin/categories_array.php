...
function makePostCatPageReturnVariable($pages = false) {
	...
	foreach ($cat_array as $cat) {
		...
		if ($pages === true) {
			...
			$page_array = getDirAsArray(CONTENT_DIR_REL . $cat, array(
				EXT_PAGE,
				EXT_HIDDEN,
				EXT_DRAFT,
				EXT_LINK
...
function checkPostCatPageReturnVariable() {
	...
	foreach ($_POST ['categories'] as $cat => $tmp) {
		...
		foreach ($_POST ['categories'] [$cat] ['position'] as $pos => $tmp) { // str_replace('/','%2F',)
			...
			$post [$cat] ['error_html'] ['cat_name'] = NULL;
			if ($_POST ['categories'] [$cat] ['position'] [$pos] > $max_cat_page + 1) {
				$post ['error_messages'] ['check_too_many_pages'] ['color'] = $error_color ['check_too_many_pages'];
			...
			if (isset($_POST ['categories'] [$cat] ['new_url'] [$pos])) {
				$post [$cat] ['new_url'] [$pos] = trim(str_replace(array(
					...
					"\n",
					"\r"
				), '', $_POST ['categories'] [$cat] ['new_url'] [$pos]));
...
function askMessages($title, $message, $input_name, $input_value, $input2_name = false, $input2_value = false) {
	...
	$ask = '<div class="message_ask">' . getLanguageValue($title) . ':<br />' . '<span style="font-weight:normal;">→&nbsp;&nbsp;' . messagesOutLen($specialchars->rebuildSpecialChars($message, true, true)) . '</span>' . '<div style="text-align:center">' . '<input type="image" name="confirm_true" value="true" alt="' . getLanguageValue("yes") . '" src="gfx/icons/' . $icon_size . '/accept.png" title="' . getLanguageValue("yes") . '" style="margin-right:10px">' . getLanguageValue("yes") . '<input type="image" name="confirm_false" value="false" alt="' . getLanguageValue("no") . '" src="gfx/icons/' . $icon_size . '/cancel.png" title="' . getLanguageValue("no") . '" style="margin-right:10px;margin-left:60px">' . getLanguageValue("no") . '<input type="hidden" name="' . $input_name . '" value="' . $input_value . '">' . $input2 . '</div></div>';
	return $ask;
}
...
function position_move($org_position, $new_position, $new_move_cat_page = array()) {
	$max_cat_page = 100;
	$array_return ['move'] = array();
