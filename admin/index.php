...
* 
* $Revision: 165 $
* $LastChangedDate: 2009-01-30 21:48:15 +0100 (Fr, 30 Jan 2009) $
* $Author: arvid $
*
*/
...
function newCategory() {
	...
	if(isset($_REQUEST["position"])) {
		...
		elseif(strlen($_REQUEST["position"])>2) {
...
function deleteCategory() {
	...
	foreach ($dirs as $file) {
		...
		&& ($fileshandle = opendir("$CONTENT_DIR_REL/".$file."/dateien"))
			...
			$pagecontent .= "<td class=\"config_row2 righttext\"><a href=\"index.php?action=deletecategory&amp;cat=$file\" class=\"imagelink\"><img src=\"gfx/actions/delete.png\" alt=\"".getLanguageValue("button_delete")."\" title=\"".getLanguageValue("button_delete")."\" /></a></td></tr>";
		}
	}
	$pagecontent .= "</table>";
...
function editSite() {
	...
	{
		if($_GET["message"] == "copysuccess")
		{
			$pagecontent .= "<span class='erfolg'>" . getLanguageValue("copymove_type_copy_success") . "</span>";
		}
		else if($_GET["message"] == "movesuccess")
		{
			$pagecontent .= "<span class='erfolg'>" . getLanguageValue("copymove_type_move_success") . "</span>";
		}
		else if($_GET["message"] == "copyfailed")
		{
			$pagecontent .= "<span class='fehler'>" . getLanguageValue("copy_error") . "</span>";
		}
	...
	if (isset($_REQUEST['file']) && isset($_REQUEST['cat'])) {
		...
		if (isset($_REQUEST['invalidvalues'])) {
			$pagecontent .= returnMessage(false, $_REQUEST['invalidname'].": ".getLanguageValue("invalid_values"));
			$tempfile = "temp.txt";
		}
		...
		$pagecontent .= "<table class=\"data\" style=\"margin:0px 0px;display:none;\"><tr>"
		...
		foreach ($dirs as $file) {
			...
			if (isValidDirOrFile($file) && ($file <> ".svn") && ($file <> ".s_n") && ($subhandle = opendir("$CONTENT_DIR_REL/".$file))) {
				...
				foreach ($catcontent as $subfile) {
					...
					$pagecontent .= "<a href=\"../index.php?cat=".$file."&amp;page=".substr($subfile, 0, strlen($subfile)-4)."$draftaction\" class=\"imagelink\" target=\"_blank\"><img src=\"gfx/actions/view.png\" alt=\"".getLanguageValue("button_preview")."\" title=\"".getLanguageValue("button_preview")."\" /></a>";
					$pagecontent .= "&nbsp;<a href=\"index.php?action=editsite&amp;cat=".$file."&amp;file=".$subfile."\" class=\"imagelink\"><img src=\"gfx/actions/editpage.png\" alt=\"".getLanguageValue("button_edit")."\" title=\"".getLanguageValue("button_edit")."\" /></a>";
					$pagecontent .= "&nbsp;<a href=\"index.php?action=copymovesite&amp;cat=".$file."&amp;file=".$subfile."\" class=\"imagelink\"><img src=\"gfx/actions/copypage.png\" alt=\"".getLanguageValue("button_copymove")."\" title=\"".getLanguageValue("button_copymove")."\" /></a>";
					$pagecontent .= "</td></tr>";
					$hasdata = true;
				}
				if (!$hasdata)
...
function copymoveSite() {
	...
	if($step > 1)
	{
		if(isset($_POST["type"]) && isset($_POST[NEW_PAGE_NAME]))
		{
			$checkdir = $specialchars->replaceSpecialChars("$CONTENT_DIR_REL/".$_POST[CURRENT_CAT]);
			$checkfile = $specialchars->replaceSpecialChars($checkdir . "/" . $_POST["position"] . "_" . $_POST[NEW_PAGE_NAME]);
			//echo $sourcefile . "|" . $checkfile;
			if(file_exists($checkdir) && !file_exists($checkfile) && strlen($_POST["position"]) == 2)
			{
				$sourcefile = $specialchars->replaceSpecialChars("$CONTENT_DIR_REL/".$_POST[CAT_PARAM] . "/" . $_POST[PAGE_PARAM],true);
				
				// Kopieren				
				//echo $sourcefile . "|" . $checkfile;
				if(@copy($sourcefile, $checkfile . substr($sourcefile,strlen($sourcefile) - 4,4)))
				{
					if($_POST["type"] == "1")
					{
						...
						if(@unlink($sourcefile))
						{
							...
							header("location:index.php?action=editsite&message=movesuccess");
							die("");
						}
					}
					header("location:index.php?action=editsite&message=copysuccess");
					die("");

				}
				else
				{
					header("location:index.php?action=editsite&message=copyfailed");
					die("");
				}
			}
			else
			{
				$pagecontent .= "<span class='fehler'>".getLanguageValue("page_exist")."</span>";
				$step = 1;
				$showstep1 = "block";
			}
		}
	...
	$pagecontent .= "<div style='display: " . $showstep0 . ";'>";
	...
	$pagecontent .= "<select name=\"" . CURRENT_CAT . "\" id=\"" . CURRENT_CAT . "\" value=\"" . $_POST[CURRENT_CAT] .  "\" class=\"maxwidth\">";
	
	// Aufbauen der Kategorien
	$dirs = array();
	$dirs = getDirs("$CONTENT_DIR_REL");
	foreach ($dirs as $element) 
	{
		...
		$selected = "";
		// Auf selektion prüfen
		if ($_POST[CURRENT_CAT] == $element."_".$specialchars->replaceSpecialChars(specialNrDir("$CONTENT_DIR_REL", $element), true))
		{
			$selected = "selected ";
		}
		$pagecontent .= "<option ".$selected."value=\"".$element."_".$specialchars->replaceSpecialChars(specialNrDir("$CONTENT_DIR_REL", $element), true)."\">".$specialchars->rebuildSpecialChars(specialNrDir("$CONTENT_DIR_REL", $element), true)."</option>";
	}

	
	$pagecontent .= "</select></td class='config_row1'></tr></table></div>";

	// STEP 1 ------------------------------------------------------------------
	$pagecontent .= "<div style='display: " . $showstep1 . ";'>";
	$pagecontent .= "<h2>".getLanguageValue("button_copymove_edit")."</h2>";
	if(isset($_POST["type"]))
	{
		$tempchecked = "checked" . $_POST["type"]; 
		$$tempchecked = "checked";
	}
	else
	{
	...
	$pagecontent .= "<table style='width: 100%;'>"
				 ...
				 .  "<input type=\"radio\" name=\"type\" value=\"1\" $checked1 />".getLanguageValue("copymove_type_move")."<br /></div>"
				 .  "</td>"
				 .	"</tr>"
				 .  "<tr>"
				 .  "<td class='config_row1'>".getLanguageValue("button_copymove_choose_pos")."</td>"
				 .  "<td class='config_row2'>"
				 .  show_files("$CONTENT_DIR_REL/". $_POST[CURRENT_CAT], str_replace("_"," ",substr($pagevalue,0,strlen($pagevalue)-4)) , false)
				 .  "</td>"
				 .  "</tr>"
				 .  "<tr>"
				 .  "<td class='config_row1'>".getLanguageValue("button_copymove_new_name")."</td>"
				 .  "<td class='config_row2'>"
				 .  "<input type='text' id='" . NEW_PAGE_NAME . "' name='" . NEW_PAGE_NAME . "' value='" . $specialchars->rebuildSpecialChars(substr($pagevalue,3,strlen($pagevalue)-7), true) . "' />"
				 .  "</td>"
				 .  "</tr>"
				 .  "</table>";

	$pagecontent .= "</div>";
	//echo "$CONTENT_DIR_REL/". $_POST[CURRENT_CAT];
	...
	foreach ($dirs as $file) {
		...
		if (isValidDirOrFile($file) && ($subhandle = opendir("$CONTENT_DIR_REL/".$file))) {
			...
			foreach ($catcontent as $subfile) {
				...
				$pagecontent .= "<tr><td class=\"config_row1\">".$specialchars->rebuildSpecialChars(substr($subfile, 3, strlen($subfile)-7), true)."$status</td><td class=\"config_row2 righttext\">";
				$pagecontent .= "<a href=\"../index.php?cat=".$file."&amp;page=".substr($subfile, 0, strlen($subfile)-4)."$draftaction\" class=\"imagelink\" target=\"_blank\"><img src=\"gfx/actions/view.png\" alt=\"".getLanguageValue("button_preview")."\" title=\"".getLanguageValue("button_preview")."\" /></a>";
				$pagecontent .= "&nbsp;<a href=\"index.php?action=deletesite&amp;cat=".$file."&amp;file=".$subfile."\" class=\"imagelink\"><img src=\"gfx/actions/delete.png\" alt=\"".getLanguageValue("button_delete")."\" title=\"".getLanguageValue("button_delete")."\" /></a>";
				$pagecontent .= "</td></tr>";
				$hasdata = true;
			}
			if (!$hasdata)
			...
			if (!file_exists("$GALLERIES_DIR_REL/".$dirname)) {
				if (@mkdir($GALLERIES_DIR_REL."/".$dirname, 0777) && @mkdir($GALLERIES_DIR_REL."/".$dirname."/".$PREVIEW_DIR_NAME, 0777)) {
					...
					if ($ADMIN_CONF->get("chmodnewfiles") == "true") {
						chmod ($GALLERIES_DIR_REL."/".$dirname, octdec($ADMIN_CONF->get("chmodnewfilesatts")));
						chmod ($GALLERIES_DIR_REL."/".$dirname."/".$PREVIEW_DIR_NAME, octdec($ADMIN_CONF->get("chmodnewfilesatts")));
					}
		...
		if (isValidDirOrFile($file) && ($subhandle = opendir("$CONTENT_DIR_REL/".$file."/dateien"))) {
			...
			foreach ($mysubfiles as $subfile) {
				if (isValidDirOrFile($subfile)) {
					...
					."<a href=\"$CONTENT_DIR_REL/".$file."/dateien/".$subfile."\" target=\"_blank\" class=\"imagelink\"><img src=\"gfx/actions/download.png\" alt=\"".getLanguageValue("data_download")."\" title=\"".getLanguageValue("data_download")."\" /></a>"
					."&nbsp;<a href=\"index.php?action=deletefile&amp;cat=".$file."&amp;file=".$subfile."\" class=\"imagelink\"><img src=\"gfx/actions/delete.png\" alt=\"".getLanguageValue("button_delete")."\" title=\"".getLanguageValue("button_delete")."\" /></a>"
					."</td></tr>";
					$hasdata = true;
				}
			}
...
*/
			...
			foreach ($contactoptions as $contactoption) {
				if (checkBoxIsChecked($_REQUEST['show_'.$contactoption]))
					$show = "true";
				else
					$show = "false";
				if (checkBoxIsChecked($_REQUEST['mandatory_'.$contactoption]))
					$mandatory = "true";
				else
					$mandatory = "false";
...
function configAdminDisplay() {
	...
	if (isset($_REQUEST['apply'])) {
		if (checkBoxIsChecked($_REQUEST['tooltip'])) {
		...
		if (isset($_REQUEST['textareaheight']) && preg_match("/^[0-9]+$/", $_REQUEST['textareaheight'])) {
		...
		if (isset($_REQUEST['backupmsgintervall']) && preg_match("/^[0-9]+$/", $_REQUEST['backupmsgintervall'])) {
			$ADMIN_CONF->set("backupmsgintervall", $_REQUEST['backupmsgintervall']);
			$changesmade = true;
		}
		if (checkBoxIsChecked($_REQUEST['overwrite'])) {
		...
		if (!isset($_REQUEST['chmodnewfiles']) || preg_match("/^[0-7]{3}$/", $_REQUEST['chmodnewfilesatts'])) {
			if (checkBoxIsChecked($_REQUEST['chmodnewfiles'])) {
		...
		if (checkBoxIsChecked($_REQUEST['sendadminmail'])) {
		...
		if (isset($_REQUEST['maxnumberofuploadfiles']) && preg_match("/^[0-9]+$/", $_REQUEST['maxnumberofuploadfiles'])) {
		...
		if(isset($_REQUEST['imagewidth']) && isset($_REQUEST['imageheight']) && $_REQUEST['imagewidth'] != "" && $_REQUEST['imageheight'] != "")
		{
			if(isValidRequestParameter("imagewidth",1) && isValidRequestParameter("imageheight",1))
			{
		...
		{
			if(isset($_REQUEST['imageheight']) && isValidRequestParameter("imageheight",1))
			{
				$ADMIN_CONF->set("maximageheight", $_REQUEST['imageheight']);			
				$ADMIN_CONF->set("maximagewidth", floor((($_REQUEST['imageheight'] / 3) * 4)));
			}
			elseif(isset($_REQUEST['imagewidth']) && isValidRequestParameter("imagewidth",1))
			{
				$ADMIN_CONF->set("maximagewidth", $_REQUEST['imagewidth']);	
		...
		if (checkBoxIsChecked($_REQUEST['resizeimages'])) {
	...
	if (isset($uploadfile) and !$uploadfile['error']) {
		...
		// alles okay, hochladen!
			...
			if($ADMIN_CONF->get("resizeimages") == "true")
			{
				...
				// GIF 
				if($size[2] == 1) { 
				  $originalpic = ImageCreateFromGIF($savepath); 
				  $resizedpic = ImageCreateTrueColor($width,$height); 
				  ImageCopyResized($resizedpic,$originalpic,0,0,0,0,$width,$height,$size[0],$size[1]); 
				  ImageGIF($resizedpic, $savepath); 
				} 
				
				// JPG 
				elseif($size[2] == 2) { 
				  $originalpic=ImageCreateFromJPEG($savepath); 
				  $resizedpic=ImageCreateTrueColor($width,$height); 
				  ImageCopyResized($resizedpic,$originalpic,0,0,0,0,$width,$height,$size[0],$size[1]); 
				  ImageJPEG($resizedpic, $savepath); 
				} 
				
				// PNG 
				elseif($size[2] == 3) { 
				  $originalpic = ImageCreateFromPNG($savepath); 
				  $resizedpic = ImageCreateTrueColor($width,$height); 
				  ImageCopyResized($resizedpic,$originalpic,0,0,0,0,$width,$height,$size[0],$size[1]); 
				  ImagePNG($resizedpic, $savepath); 
				} 
			}
...
function checkBoxIsChecked($checkboxrequest) {
	return (isset($checkboxrequest) && ($checkboxrequest == "on"));
}
