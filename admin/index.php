...
if (isset($_REQUEST ['javascript']) and $_REQUEST ['javascript'] == "ja")
	$_POST ['javascript'] = "ja";
...
function home($post) {
	...
	// CMS-INFOS
	"<tr>" . '<td width="100%" class="td_cms_titel" colspan="2"><b>' . getLanguageValue("cmsinfo") . '</b></td>' . "</tr>" . 
	// Zeile "CMS-VERSION"
	"<tr>" . '<td width="50%" class="td_cms_left">' . getLanguageValue("cmsversion_text") . "</td>" . '<td width="50%" class="td_cms_left">' . $VERSION_CONF->get("cmsversion") . ' ("' . $VERSION_CONF->get("cmsname") . '")<br />' . getLanguageValue("cmsrevision_text") . ' ' . $VERSION_CONF->get("revision") . '</td>' . "</tr>" . 
	// Zeile "Gesamtgröße des CMS"
	"<tr>" . '<td width="50%" class="td_cms_left">' . getLanguageValue("cmssize_text") . "</td>" . '<td width="50%" class="td_cms_left">' . $cmssize . "</td>" . "</tr>" . 
	// SERVER-INFOS
	"<tr>" . '<td width="100%" class="td_cms_titel" colspan="2"><b>' . getLanguageValue("serverinfo") . '</b></td>' . "</tr>" . 
	// Zeile "Installationspfad"
	"<tr>" . '<td width="50%" class="td_cms_left">' . getLanguageValue("installpath_text") . "</td>" . '<td width="50%" class="td_cms_left">' . $path . "</td>" . "</tr>" . 
	// Zeile "PHP-Version"
	"<tr>" . '<td width="50%" class="td_cms_left">' . getLanguageValue("phpversion_text") . "</td>" . '<td width="50%" class="td_cms_left">' . phpversion() . "</td>" . "</tr>" . 
	// Zeile "Safe Mode"
	"<tr>" . '<td width="50%" class="td_cms_left">' . getLanguageValue("home_text_safemode") . "</td>" . '<td width="50%" class="td_cms_left">' . $safemode . "</td>" . "</tr>" . 
	// Zeile "GDlib installiert"
	"<tr>" . '<td width="50%" class="td_cms_left">' . getLanguageValue("home_text_gd") . "</td>" . '<td width="50%" class="td_cms_left">' . $gdlibinstalled . "</td>" . "</tr>" . 
	// test
	"<tr>" . '<td width="100%" class="td_cms_titel" colspan="2"><b>' . getLanguageValue("home_titel_mod_rewrite") . '</b></td>' . "</tr>" . "<tr>" . '<td width="50%" class="td_cms_left">' . getLanguageValue("home_text_mod_rewrite") . "</td>" . '<td width="50%" class="td_cms_left">' . $modrewrite . '</td>' . "</tr>" . "<tr>" . '<td width="100%" class="td_cms_titel" colspan="2"><b>' . getLanguageValue("home_titel_test_mail") . '</b></td>' . "</tr>" . "<tr>" . '<td width="50%" class="td_cms_left">' . getLanguageValue("home_text_test_mail") . "</td>" . '<td width="50%" class="td_cms_left"><input type="submit" class="input_submit" name="test_mail" value="' . getLanguageValue("home_input_test_mail") . '">&nbsp;&nbsp;&nbsp;<input type="text" class="input_mail" name="test_mail_adresse" value="' . $test_mail_adress . '"></td>' . "</tr>" . "</table>";
	return array(
		getLanguageValue("home_button"),
		$pagecontent
	);
}
...
function category($post) {
	...
	$array_getLanguageValue = array(
		...
		"url_adress",
		"url_new_adress",
		"url_adress_description",
		"contents",
		"category_button_change",
		"category_button_delete",
		"target",
		"toggle_show",
		"toggle_hide"
	);
	...
	$array_getTooltipValue = array(
		"help_new_url",
		"help_target_blank",
		"help_target_self",
		"help_target",
		"help_url",
		"category_help_new_position",
		"category_help_new_name",
		"category_help_position",
		"category_help_delete",
		"category_help_name",
		"category_help_edit"
	);
	...
	foreach ($post ['categories'] ['cat'] ['position'] as $pos => $position) {
		...
		if (!isset($display_new_cat)) {
			...
			if (count($post ['categories'] ['cat'] ['position']) < $max_cat_page + 1) {
				$pagecontent .= '<tr><td width="100%" class="td_toggle_new">' . '<table summary="" width="100%" class="table_new" border="0" cellspacing="0" cellpadding="0">' . '<tr><td class="td_left_title"><b>' . $text_position . '</b></td>' . '<td class="td_left_title"><b>' . $text_name . '</b>' . '</td><td class="td_left_title"><b>' . $text_url_adress . '</b> ' . $text_url_adress_description . '</td>' . '<td class="td_center_title">&nbsp;</td>' . '<td class="td_center_title">' . getLanguageValue("blank") . '</td>' . '<td class="td_center_title">' . getLanguageValue("self") . '</td></tr>';
				$pagecontent .= '<tr>';
				...
				$pagecontent .= '<td class="td_center_title"' . $tooltip_help_target . '><b>' . $text_target . '</b></td>' . '<td class="td_center_title"><input type="radio" name="categories[cat][new_target][' . $max_cat_page . ']" value="-_blank-"' . $post ['categories'] ['cat'] ['checked_blank'] [$max_cat_page] . $tooltip_help_target_blank . '></td>';
	...
	return array(
		getLanguageValue("category_button"),
		$pagecontent
	);
}
...
function page($post) {
	...
	$array_getTooltipValue = array(
		...
		"page_help_edit",
		"page_help_position",
		"page_help_new_position",
		"page_help_name",
		"page_help_new_name",
		"page_help_move",
		"page_help_editieren",
		"page_help_edit_pages",
		"page_help_delete",
		"page_help_copy",
		"page_help_password",
		"page_help_password_del",
		"page_page_help_show",
		"page_link_help_show"
	);
	...
	foreach ($post ['categories'] as $cat => $tmp) {
		...
		foreach ($post ['categories'] [$cat] ['position'] as $pos => $tmp) {
			...
			if (!isset($display_new_cat)) {
				...
				$pagecontent .= '<tr><td class="td_left_title"><b>' . $text_position . '</b></td>' . '<td class="td_left_title"><b>' . $text_name . '</b></td>' . '<td class="td_left_title"><b>' . $text_url_adress . '</b> ' . $text_url_adress_description . '</td>' . '<td class="td_center_title">&nbsp;</td>' . '<td class="td_center_title">' . getLanguageValue("blank") . '</td>' . '<td class="td_center_title">' . getLanguageValue("self") . '</td></tr>';
				$pagecontent .= '<tr>';
			...
			if (isset($post ['categories'] [$cat] ['url'] [$pos])) {
				...
				$pagecontent .= '<td class="td_left_title">';
				$pagecontent .= '<input type="text" class="input_readonly" name="categories[' . $cat . '][url][' . $pos . ']" value="' . $specialchars->rebuildSpecialChars($post ['categories'] [$cat] ['url'] [$pos], true, true) . '" maxlength="' . $max_strlen . '" readonly>';
				$pagecontent .= '</td>' . '<td class="td_center_title">&nbsp;</td>' . '<td valign="bottom" class="td_center_title">' . getLanguageValue("blank") . '</td>' . '<td valign="bottom" class="td_center_title">' . getLanguageValue("self") . '</td>';
				$pagecontent .= '</tr><tr>';
				$pagecontent .= '';
				...
				$pagecontent .= '<td class="td_center_title"' . $tooltip_help_target . '><b>' . $text_target . '</b></td>' . '<td width="6%" class="td_center_title"><input type="radio" name="categories[' . $cat . '][new_target][' . $pos . ']" value="-_blank-"' . $post ['categories'] [$cat] ['checked_blank'] [$pos] . '' . $tooltip_help_target_blank . '></td>';
				...
				$pagecontent .= '<tr><td class="td_left_title" nowrap><input type="text" ' . $post ['categories'] [$cat] ['error_html'] ['new_name'] [$pos] . ' class="input_text" name="categories[' . $cat . '][new_name][' . $pos . ']" value="' . $specialchars->rebuildSpecialChars($post ['categories'] [$cat] ['new_name'] [$pos], true, true) . '" maxlength="' . $max_strlen . '"' . $tooltip_page_help_name . '></td>' . '<td>&nbsp;</td>' . '<td class="td_left_title"><input type="text" ' . $post ['categories'] [$cat] ['error_html'] ['password'] [$pos] . ' class="input_text" name="categories[' . $cat . '][password][' . $pos . ']" value="' . $specialchars->rebuildSpecialChars($post ['categories'] [$cat] ['password'] [$pos], true, true) . '" maxlength="' . $max_strlen . '"' . $tooltip_page_help_password . '></td>' . '<td class="td_left_title">';
...
function editSite($post) {
	...
	if (isset($_POST ['save']) or isset($_POST ['savetemp'])) {
		...
		if (isset($_POST ['save']) and !isset($post ['error_messages']) and isset($_POST ['edit_next_page']) and !empty($_POST ['edit_next_page'])) {
...
function copymoveSite($post) {
	...
	foreach ($post ['categories'] as $cat => $tmp) {
		...
		foreach ($post ['categories'] [$cat] ['position'] as $pos => $tmp) {
			...
			if ($PASSWORDS->get($cat . "/" . $tmp_page) and !empty($post ['categories'] [$cat] ['password_del'] [$pos])) {
				$post ['display'] [$cat] ['error_html'] ['display_cat'] = 'style="display:block;" ';
				$post ['messages'] ['page_message_del_password'] [] = $tmp_page;
				$PASSWORDS->delete($cat . "/" . $tmp_page);
			}
			if (!$PASSWORDS->get($cat . "/" . $tmp_page) and !empty($post ['categories'] [$cat] ['password'] [$pos]) and empty($post ['categories'] [$cat] ['password_del'] [$pos])) {
				$post ['display'] [$cat] ['error_html'] ['display_cat'] = 'style="display:block;" ';
				$post ['display'] [$cat] ['error_html'] ['display'] [$pos] = 'style="display:block;" ';
			...
			} elseif ($PASSWORDS->get($cat . "/" . $tmp_page) != $pwcrypt->encrypt($post ['categories'] [$cat] ['password'] [$pos]) and !empty($post ['categories'] [$cat] ['password'] [$pos]) and empty($post ['categories'] [$cat] ['password_del'] [$pos])) {
				$post ['display'] [$cat] ['error_html'] ['display_cat'] = 'style="display:block;" ';
				$post ['display'] [$cat] ['error_html'] ['display'] [$pos] = 'style="display:block;" ';
	...
	if (isset($new_page)) {
		foreach ($new_page as $cat => $tmp) {
			...
			if (substr($new_page [$cat], -4) == EXT_DRAFT) {
				$page_inhalt = "[ueber1|" . str_replace(array(
					"[",
					"]"
				), array(
					"^[",
					"^]"
				), $specialchars->rebuildSpecialChars(substr($new_page [$cat], 3, strlen($new_page [$cat]) - 7), false, false)) . "]";
			}
...
function gallery($post) {
	...
	foreach ($dircontent as $pos => $currentgalerien) {
		...
		foreach ($gallerypics [$currentgalerien] as $pos => $file) {
			// nur Bilder zulassen
			$imgSizeData = @getimagesize(GALLERIES_DIR_REL . $currentgalerien . '/' . $file);
			$isImage = $imgSizeData !== false;
			if (is_dir($file) or !$isImage) {
	...
	$array_getLanguageValue = array(
		"gallery_scale",
		"gallery_scale_thumbs",
		"gallery_create_thumbs",
		"gallery_scaleimages",
		"gallery_rebuildthumbs",
		"gallery_size",
		"gallery_subtitle",
		"gallery_button_cut",
		"gallery_newname",
		"gallery_button_img_delete",
		"gallery_button_gallery_delete",
		"target",
		"toggle_show",
		"toggle_hide",
		"gallery_no_preview"
	...
	$array_getTooltipValue = array(
		...
		"help_target",
		"gallery_help_scale",
		"gallery_help_scale_thumbs",
		"gallery_help_input_scale",
		"gallery_help_create_thumbs",
		"gallery_help_all_picture_scale",
		"gallery_help_all_thumbs_new",
		"gallery_help_size",
		"gallery_help_picture",
		...
		"gallery_help_conf",
		"gallery_help_edit",
		"gallery_help_newname",
		"gallery_help_usedfgallery",
		"gallery_help_overwrite"
	);
	...
	foreach ($dircontent as $pos => $currentgalerien) {
		...
		foreach ($gallerypics [$currentgalerien] as $pos => $file) {
			...
			if (file_exists(GALLERIES_DIR_REL . $currentgalerien . "/" . PREVIEW_DIR_NAME . "/" . $file)) {
				$preview = '<a href="' . $specialchars->replaceSpecialChars(URL_BASE . GALLERIES_DIR_NAME . "/" . $currentgalerien . "/" . $file, true) . '" target="_blank"' . $tooltip_gallery_help_picture . '>' . '<img src="' . $specialchars->replaceSpecialChars(URL_BASE . GALLERIES_DIR_NAME . "/" . $currentgalerien . "/" . PREVIEW_DIR_NAME . "/" . $file, true) . '" alt="' . $specialchars->rebuildSpecialChars($file, true, true) . '" style="height:60px;border:none;" /></a>';
			} else {
	...
	$pagecontent .= '</table>';
	return array(
		getLanguageValue("gallery_button"),
		$pagecontent
	);
}
...
function editGallery($post) {
	...
	foreach ($post ['gallery'] as $gallery => $gallery_array) {
		if ($gallery == "setings" or $gallery == "error_html" or $gallery == "make_thumbs" or $gallery == "scale_max")
			continue;
		// Neuer Gallery Name
...
function deleteGallery($post) {
	...
	if (!isset($_POST ['confirm'])) {
		...
		$del_gal = $post ['action_data'] ['deletegallery'] [$del_gallery];
		$post ['ask'] = askMessages("gallery_ask_delete", $del_gallery, 'action_data[deletegallery]', $post ['action_data'] ['deletegallery'] [$del_gallery], "del_gallery", $del_gallery);
	}
	// Gallery Löschen
...
function files($post) {
	...
	$array_getTooltipValue = array(
		...
		"files_help_show",
		"files_help_upload",
		"files_help_downloads",
		"files_help_overwrite"
	);
	...
	foreach ($post ['categories'] ['cat'] ['position'] as $pos => $position) {
		...
		if (isValidDirOrFile($file) && ($subhandle = @opendir(CONTENT_DIR_REL . $file . "/" . CONTENT_FILES_DIR_NAME))) {
			...
			$pagecontent .= '<tr><td class="td_left_title_padding_bottom td_toggle_new" colspan="1">' . $text_files_text_upload . '</td>' . '<td colspan="4" class="td_right_title_padding_bottom td_toggle_new"' . $tooltip_files_help_upload . '><input type="file" id="uploadfileinput_' . $pos . '" name="uploadfile" class="uploadfileinput"></td></tr>' . '<tr><td colspan="5" class="td_right_title_padding_bottom td_toggle"><div id="files_list_' . $pos . '" class="text_cat_page"></div>';
			...
			foreach ($mysubfiles as $subfile) {
				if (isValidDirOrFile($subfile)) {
					...
					$pagecontent .= $titel_dateien . '<tr><td class="td_left_title_padding_bottom"><a class="file_link" href="' . URL_BASE . CONTENT_DIR_NAME . '/' . $specialchars->replaceSpecialChars($file . '/dateien/' . $subfile, true) . '" target="_blank"' . $tooltip_files_help_show . '>' . $specialchars->rebuildSpecialChars($subfile, true, true) . '</a></td>' . '<td class="td_left_title_padding_bottom" nowrap><span class="text_info">' . convertFileSizeUnit($filesize) . '</span></td>' . '<td class="td_left_title_padding_bottom" nowrap><span class="text_info">' . @strftime(getLanguageValue("_dateformat"), $uploadtime) . '</span></td>' . '<td class="td_center_title_padding_bottom" nowrap><span class="text_info">' . $downloads . " " . $downloadsperdaytext . '</span></td>';
...
function config($post) {
	...
	foreach ($cat_array as $element) {
		if (count(getDirAsArray(CONTENT_DIR_REL . $element, array(
			EXT_PAGE
		), "none")) == 0) {
			continue;
		}
	...
	if ($ADMIN_CONF->get('showexpert') == "true") {
		...
		$pagecontent .= "<td class=\"td_cms_left\">" . buildCheckBox("usecmssyntax", $CMS_CONF->get("usecmssyntax")) . getLanguageValue("config_input_usesyntax");
		$pagecontent .= "</td></tr>";
	}
	...
	if ($CMS_CONF->get("usecmssyntax") == "true") {
		if ($ADMIN_CONF->get('showexpert') == "true") {
			...
			$pagecontent .= "<td class=\"td_cms_left\">" . buildCheckBox("usecmseditarea", $CMS_CONF->get("usecmseditarea"));
			$pagecontent .= "</td></tr>";
			// Zeile "LINKS KÜRZEN"
			$checked0 = "";
			$checked1 = "";
		...
		if ($ADMIN_CONF->get('showexpert') == "true") {
			...
			$pagecontent .= "<td class=\"td_cms_left\">" . buildCheckBox("replaceemoticons", ($CMS_CONF->get("replaceemoticons") == "true")) . getLanguageValue("config_input_replaceemoticons") . "</td>";
			$pagecontent .= "</tr>";
		}
	}
	/*
	...
	return array(
		getLanguageValue("config_button"),
		$pagecontent
	);
}
...
function admin($post) {
	...
	$array_getTooltipValue = array(
		"admin_help_language",
		"admin_help_adminmail",
		"admin_help_chmodnewfiles",
		"admin_help_chmodupdate"
	);
	...
	if ($LOGINCONF->get("initialsetup") == "false") {
		...
		$pagecontent .= '<td class="td_cms_left">' . buildCheckBox("showexpert", ($ADMIN_CONF->get("showexpert") == "true")) . '<span ' . $error_color ['showexpert'] . '>' . getLanguageValue("admin_input_showexpert") . "</span></td>";
		$pagecontent .= "</tr>";
		// Zeile "ZEIGE TOOLTIPS"
		if ($ADMIN_CONF->get('showexpert') == "true") {
			$pagecontent .= '<tr><td class="td_cms_left">' . getLanguageValue("admin_text_tooltips") . "</td>";
			$pagecontent .= '<td class="td_cms_left">' . buildCheckBox("showTooltips", ($ADMIN_CONF->get("showTooltips") == "true")) . getLanguageValue("admin_input_tooltips") . "</td>";
			$pagecontent .= "</tr>";
		}
	}
	// Zeile "SPRACHAUSWAHL"
	...
	if ($LOGINCONF->get("initialsetup") == "false") {
		...
		if (isMailAvailable()) {
			...
			$pagecontent .= '<td class="td_cms_left">' . buildCheckBox("sendadminmail", ($ADMIN_CONF->get("sendadminmail") == "true")) . getLanguageValue("admin_input_adminmail");
			$pagecontent .= "</td>";
			$pagecontent .= "</tr>";
		...
		$pagecontent .= '<td class="td_cms_left"><input type="text" class="input_text" name="chmodnewfilesatts" value="' . $ADMIN_CONF->get("chmodnewfilesatts") . '"' . $tooltip_admin_help_chmodnewfiles . $error_color ['chmodnewfilesatts'] . ' /><br />' . buildCheckBox("chmodupdate", false) . '<span' . $tooltip_admin_help_chmodupdate . '>' . getLanguageValue("admin_input_chmodupdate") . '</span>' . '';
		$pagecontent .= '</td>';
		$pagecontent .= "</tr>";
		if ($ADMIN_CONF->get('showexpert') == "true") {
			...
			$pagecontent .= '<td class="td_cms_left">' . buildCheckBox("overwriteuploadfiles", ($ADMIN_CONF->get("overwriteuploadfiles") == "true")) . getLanguageValue("admin_input_defaultoverwrite") . '</td>';
			$pagecontent .= "</tr>";
		}
		// BILD-EINSTELLUNGEN
		if (extension_loaded("gd")) {
	...
	// Zeile "NEUER NAME"
	'<tr><td class="td_cms_right">' . getLanguageValue("pw_titel_newname") . '</td>' . '<td class="td_cms_left"><input type="text" class="input_text" name="newname" value=""' . $error_color ['newname'] . ' /></td>' . "</tr>" . 
	// Zeile "NEUES PASSWORT"
	'<tr><td class="td_cms_right">' . getLanguageValue("pw_titel_newpw") . '</td>' . '<td class="td_cms_left"><input type="password" class="input_text" value="' . NULL . '" name="newpw"' . $error_color ['newpw'] . ' /></td>' . "</tr>" . 
	// Zeile "NEUES PASSWORT - WIEDERHOLUNG"
	'<tr><td class="td_cms_right">' . getLanguageValue("pw_titel_newpwrepeat") . '</td>' . '<td class="td_cms_left"><input type="password" class="input_text" value="" name="newpwrepeat"' . $error_color ['newpwrepeat'] . ' /></td>' . "</tr>";
...
function plugins($post) {
		...
		if (file_exists(PLUGIN_DIR_REL . $currentelement . "/index.php")) {
			...
			if (class_exists($currentelement)) {
				...
				if ($plugin_error === false) {
					...
					$plugins_info_array = array(
						"Plugin_name",
						"plugins_titel_version",
						"plugins_titel_description",
						"plugins_titel_author",
						"plugins_titel_web"
					);
					foreach ($plugins_info_array as $pos => $info) {
						...
						if ($pos == 0)
							continue;
						if ($pos == 2) {
							$plugin_info [$pos] = str_replace(array(
								"&lt;",
								"&gt;"
							), array(
								"<",
						...
						if (isset($plugin_info [$pos])) {
							$pagecontent_conf .= '<tr><td width="30%" valign="top" class="td_right_title_padding_bottom" nowrap><b class="text_grau">' . getLanguageValue($info) . '</b></td>' . '<td width="70%" class="td_togglen_padding_bottom">' . $plugin_info [$pos] . '</td></tr>';
						}
						if ($pos == 4) {
					...
					if (count($config) >= 1) {
						...
						foreach ($config as $name => $inhalt) {
							...
							$value = NULL;
							$pluginConfig = $conf_plugin->get($name);
							if (isset($pluginConfig) && strlen($pluginConfig) > 0) {
								// in einem input feld darf der inhalt keine " haben
								$value = ' value="' . str_replace('"', "&quot;", $pluginConfig) . '"';
	...
	$pagecontent .= '</table>';
	return array(
		getLanguageValue("plugins_button"),
		$pagecontent
	);
}
...
function showEditPageForm($cat, $page, $newsite) {
	...
	$content .= '<input type="radio" name="saveas" value="' . EXT_PAGE . '"' . $checkednormal . ' accesskey="n" /> ' . getLanguageValue("page_saveasnormal") . ' <input type="radio" name="saveas" value="' . EXT_HIDDEN . '"' . $checkedhidden . ' accesskey="v" /> ' . getLanguageValue("page_saveashidden") . ' <input type="radio" name="saveas" value="' . EXT_DRAFT . '"' . $checkeddraft . ' accesskey="e" /> ' . getLanguageValue("page_saveasdraft");
...
function returnUserSyntaxSelectbox() {
	...
	$content = "<select class=\"usersyntaxselectbox\" name=\"usersyntax\" onchange=\"insertTagAndResetSelectbox(this);\">" . "<option class=\"noaction\" value=\"\">" . getLanguageValue("toolbar_usersyntax") . "</option>";
...
function returnPlatzhalterSelectbox() {
	...
	$selectbox = '<select class="overviewselect" name="platzhalter" onchange="insertAndResetSelectbox(this);">' . '<option class="noaction" value="">' . getLanguageValue("toolbar_platzhalter") . '</option>';
...
function returnPluginSelectbox() {
	...
	$selectbox = '<select class="overviewselect" name="plugins" onchange="insertPluginAndResetSelectbox(this);">' . '<option class="noaction" value="">' . getLanguageValue("toolbar_plugins") . '</option>';
...
function returnFormatToolbar($currentcat) {
	...
	$content = "<noscript>" . returnMessage(false, getLanguageValue("toolbar_nojs_text")) . "</noscript>" . '<table summary="" width="99%" cellspacing="2" border="0" cellpadding="0">' . "<tr>" . 
	// Überschrift Syntaxelemente
	"<td width=\"59%\" nowrap>" . getLanguageValue("toolbar_syntaxelements") . "</td>" . 
	// Überschrift Textformatierung
	"<td width=\"29%\" nowrap>" . getLanguageValue("toolbar_textformatting") . "</td>" . 
	// Überschrift Farben
	"<td width=\"11%\" nowrap>" . getLanguageValue("toolbar_textcoloring") . "</td>" . "</tr>" . "<tr>" . 
	// Syntaxelemente
	"<td nowrap>" . returnFormatToolbarIcon("link") . returnFormatToolbarIcon("mail") . returnFormatToolbarIcon("seite") . returnFormatToolbarIcon("kategorie") . returnFormatToolbarIcon("datei") . returnFormatToolbarIcon("bild") . returnFormatToolbarIcon("bildlinks") . returnFormatToolbarIcon("bildrechts") . returnFormatToolbarIcon("ueber1") . returnFormatToolbarIcon("ueber2") . returnFormatToolbarIcon("ueber3") . returnFormatToolbarIcon("absatz") . returnFormatToolbarIcon("liste") . returnFormatToolbarIcon("numliste") . "<img class=\"js\" alt=\"Tabelle\" title=\"[tabelle| ... ] - " . getLanguageValue("toolbar_desc_tabelle") . "\" src=\"gfx/jsToolbar/tabelle.png\" onClick=\"insert('[tabelle|\\n<< ', ' |  >>\\n<  |  >\\n]', true)\"" . createUserTooltipWZ(getLanguageValue("toolbar_desc_tabelle"), "[tabelle|<<  |  >> <  |  >]") . ">" . "<img class=\"js\" alt=\"Horizontale Linie\" title=\"[----] - " . getLanguageValue("toolbar_desc_linie") . "\" src=\"gfx/jsToolbar/linie.png\" onClick=\"insert('[----]', '', false)\"" . createUserTooltipWZ(getLanguageValue("toolbar_desc_linie"), '[----]') . ">" . returnFormatToolbarIcon("html") . returnFormatToolbarIcon("include") . "</td>" . 
	// Textformatierung
	"<td nowrap>" . returnFormatToolbarIcon("links") . returnFormatToolbarIcon("zentriert") . returnFormatToolbarIcon("block") . returnFormatToolbarIcon("rechts") . returnFormatToolbarIcon("fett") . returnFormatToolbarIcon("kursiv") . returnFormatToolbarIcon("unter") . returnFormatToolbarIcon("durch") . "</td>" . 
	// Farben
	"<td nowrap>" . '<table summary="" cellspacing="0" border="0" cellpadding="0"><tr><td>' . "<img class=\"js\" style=\"background-color:#AA0000\" alt=\"Farbe\" id=\"farbicon\" title=\"[farbe=RRGGBB| ... ] - " . getLanguageValue("toolbar_desc_farbe") . "\" src=\"gfx/jsToolbar/farbe.png\" onClick=\"insert('[farbe=' + document.getElementById('farbcode').value + '|', ']', true)\"" . createUserTooltipWZ(getLanguageValue("toolbar_desc_farbe"), '[farbe=RRGGBB| ... ]') . ">" . "</td><td nowrap>" . "<div class=\"colordiv\">" . "<input type=\"text\" readonly=\"readonly\" maxlength=\"6\" value=\"AA0000\" class=\"colorinput\" id=\"farbcode\" size=\"0\">" . "<img class=\"colorimage\" src=\"js_color_picker_v2/images/select_arrow.gif\" onmouseover=\"this.src='js_color_picker_v2/images/select_arrow_over.gif'\" onmouseout=\"this.src='js_color_picker_v2/images/select_arrow.gif'\" onclick=\"showColorPicker(this,document.getElementById('farbcode'))\" alt=\"...\" title=\"Farbauswahl\" />" . "</div>" . "</td></tr></table>" . "</td>" . "</tr>" . "</table>" . '<table summary="" width="99%" cellspacing="2" border="0" cellpadding="0">' . "<tr>";

	// Benutzerdefinierte Syntaxelemente vorbereiten
	$usersyntaxarray = $USER_SYNTAX->toArray();

	// Überschrift Inhalte
	$content .= "<td width=\"66%\" colspan=\"3\">" . getLanguageValue("toolbar_contents") . "</td>";
	// Überschrift Benutzerdefinierte Syntaxelemente
	if (count($usersyntaxarray) > 0) {
		$content .= "<td width=\"33%\">" . getLanguageValue("toolbar_usersyntax") . "</td>";
	}
	$content .= "</tr>" . "<tr>" . 
	// Inhalte
	"<td width=\"22%\">" . returnOverviewSelectbox(1, $currentcat) . "</td>" . "<td width=\"22%\">" . returnOverviewSelectbox(2, $currentcat) . "</td>" . "<td width=\"22%\">" . returnOverviewSelectbox(3, $currentcat) . "</td>";
	// Benutzerdefinierte Syntaxelemente
	if (count($usersyntaxarray) > 0) {
		$content .= "<td>" . returnUserSyntaxSelectbox() . "</td>";
	}
	$content .= "</tr>" . "</table>";
	...
	// Plugins
	$content .= '<table summary="" width="66%" cellspacing="2" border="0" cellpadding="0">' . '<tr><td width="66%" colspan="2">' . getLanguageValue("toolbar_platzhalter") . '</td></tr>' . '<tr><td width="33%">' . returnPluginSelectbox() . '</td>' . '<td width="33%">' . returnPlatzhalterSelectbox() . '</td>' . '</tr></table>';
	...
	switch ($type) {
		...
		case ($type == 1 or $type == 4):
			...
			foreach ($categories as $catdir) {
				...
				$files = $CatPage->get_PageArray($catdir, array(
					EXT_PAGE,
					EXT_HIDDEN
				), true);
				foreach ($files as $file) {
					...
					if ($type == 4) {
						$elements [] = array(
							$spacer . $completepagename,
							$CatPage->get_CatPageWithPos($catdir, false) . "/" . $CatPage->get_CatPageWithPos($catdir, $file) . $CatPage->get_Type($catdir, $file)
						);
					} else {
		...
		case 2:
			...
			foreach ($categories as $catdir) {
				...
				foreach ($currentcat_filearray as $current_file) {
					if ($catdir == $currentcat)
						$elements [] = array(
							$spacer . $specialchars->rebuildSpecialChars($current_file, true, true),
							$specialchars->rebuildSpecialChars($current_file, true, true)
						);
					else
						$elements [] = array(
							$spacer . $specialchars->rebuildSpecialChars($current_file, true, true),
							$cleancatname . ":" . $specialchars->rebuildSpecialChars($current_file, true, true)
						);
				}
			}
			$selectname = "files";
		...
		case 3:
			...
			foreach ($galleries as $currentgallery) {
				$elements [] = array(
					$specialchars->rebuildSpecialChars($currentgallery, false, true),
					$specialchars->rebuildSpecialChars($currentgallery, false, false)
				);
			}
			$selectname = "gals";
			break;
...
function uploadFile($uploadfile, $destination, $forceoverwrite, $MAX_IMG_WIDTH, $MAX_IMG_HEIGHT) {
	...
	if (isset($uploadfile) and !$uploadfile ['error']) {
		...
		elseif (file_exists($destination . $uploadfile_name) && ($forceoverwrite != "on")) {
			return array(
				"files_error_exists" => $uploadfile_name
			);
			...
			if ($last_error ['line'] == $line_error) {
				...
				return array(
					"files_error_upload" => $uploadfile_name
				);
			}
			...
			if ((!empty($MAX_IMG_WIDTH) or !empty($MAX_IMG_HEIGHT)) and preg_match("/^(\w)+(.gif|.png|.jpeg|.jpg)$/", strtolower($uploadfile_name))) {
				...
				if ($image_typ == "gif" or $image_typ == "png" or $image_typ == "jpeg") {
					...
					return array(
						"files_error_no_image" => $uploadfile_name
					);
				}
			}
			return;
