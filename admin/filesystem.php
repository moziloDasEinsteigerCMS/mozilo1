		...
		if (isValidDirOrFile($currentcategory)) {
			...
			while ($currentpage = readdir($cathandle)) {
				if (isValidDirOrFile($currentpage) && is_file(CONTENT_DIR_REL . $currentcategory . "/" . $currentpage)) {
					...
					if (count($matches_1) == 0 and count($matches_2) == 0)
						continue;
...
function updateReferencesInText($currentPagesContent, $currentPagesCategory, $movedPage, $oldCategory, $oldPage, $newCategory, $newPage, $allowed_attributes) {
	...
	$hack_eckigeklamern = str_replace(array(
		"[",
		"]"
	), array(
		"&#94;[",
	...
	$currentPagesContentmatches = str_replace(array(
		"^[",
		"^]"
	), array(
		"&#94;&#091;",
	...
	foreach ($matches [0] as $i => $match) {
		...
		if (in_array($allowed_test, $allowed_attributes)) {
			if ($debug)
				echo "match = $match -----------<br />\n";
			if ($debug)
				echo "datei = $pos_currentPagesCategory/$movedPage<br />\n";
			...
			if (!empty($oldCategory) and !empty($newCategory) and empty($oldPage) and empty($newPage)) {
				...
				if (strstr($match, "|" . $oldCategory . ":") or strstr($match, "|" . $oldCategory . "]")) {
					...
					if ($debug)
						echo "cat = $match -> $replace_match<br />\n";
				}
			}
			...
			if (!empty($oldCategory) and empty($newCategory) and !empty($oldPage) and !empty($newPage)) {
				...
				if ((strstr($match, "|$oldCategory:$oldPage]") or (strstr($match, "|$oldPage]") and $pos_oldCategory == $pos_currentPagesCategory))) {
					$replace_match = str_replace($oldPage, $newPage, $match);
					if ($debug)
						echo "page = $match -> $replace_match<br />\n";
				}
			}
			...
			if (!empty($oldCategory) and !empty($newCategory) and !empty($oldPage) and !empty($newPage)) {
				...
				if ($movedPage == $pos_newPage and !strstr($match, ":") and $oldCategory != $newCategory) {
				  ...
				  // Kategorie entfernen
				elseif ($movedPage == $pos_newPage and strstr($match, ":") and $pos_currentPagesCategory == $pos_newCategory) {
			...
			if (!empty($replace_match) and $matches [0] [$i] != $replace_match) {
				...
				if ($debug)
					echo "diff == match = " . $matches [0] [$i] . " | replace_match = $replace_match<br />\n";
				$changesmade = true;
			}
			if ($debug)
				echo "<br />\n";
...
function getChmod($dir = false) {
	...
	if (strlen($mode) > 0) {
		if ($dir === true) {
			...
			if (substr($mode, 0, 1) >= 2 and substr($mode, 0, 1) <= 6)
				$mode = $mode + 100;
			if (substr($mode, 1, 1) >= 2 and substr($mode, 1, 1) <= 6)
				$mode = $mode + 10;
			if (substr($mode, 2, 1) >= 2 and substr($mode, 2, 1) <= 6)
				$mode = $mode + 1;
		}
		return octdec($mode);
	}
